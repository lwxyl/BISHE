...
const char *password p= "123456789";
...
{ /* Take actions when received an mqtt massage */
    ...
    {
        DynamicJsonDocument doc(35); /* https://arduinojson.org/assistant/ */
        DeserializationError error = deserializeJson(doc, payload);
        if (error)
        {
            Serial.print(F("deserializeJson() failed with code "));
...
void upLoadHT(String HTtype)
{
    ...
    DynamicJsonDocument doc(50); /* https://arduinojson.org/assistant/ */
    if (HTtype == "H")
    {
        doc["H"] = h;
    }
