...
#define HTSENDTOPIC "HT/send"
#define HTCOMMANDTOPIC "HT/command"
...
void TaskPrintHT(void *pvParameters);
...
{ /* Take actions when received an mqtt massage */
    ...
    {
        DynamicJsonBuffer jsonBuffer(200); /* https://arduinojson.org/assistant/ */
        ...
        ledTurnOn(r, g, b);
        premillis = millis();
        sendCode(root["type"], valu, root["length"]);
        return;
    }
    if (stopic == HTCOMMANDTOPIC)
    {
        DynamicJsonBuffer jsonBuffer(35); /* https://arduinojson.org/assistant/ */
        JsonObject &root = jsonBuffer.parseObject(payload);
        if (!root.success())
        { /* Test if parsing succeeds */
            Serial.println(F("parseObject() failed"));
        ...
        printPayload(root);
        upLoadHT(root["HTtype"]);
        return;
    }
}

void upLoadHT(String HTtype)
{
    String HTsend = "";
    DynamicJsonBuffer jsonBuffer(50); /* https://arduinojson.org/assistant/ */
    JsonObject &root = jsonBuffer.createObject();
    if (HTtype == "H")
    {
        root["H"] = dht.readHumidity();
    }
    else if (HTtype == "T")
    {
        root["T"] = dht.readTemperature();
    }
    else if (HTtype == "HT")
    {
        root["H"] = dht.readHumidity();
    ...
    {
        Serial.println(F("HT payload error"));
        return;
    }
    root.printTo(HTsend);
    printPayload(root);
    publishMqtt(HTSENDTOPIC, HTsend);
}
void printPayload(JsonObject &root)
{
...
void connectMqttServer()
{
    ...
    { /* Loop until we're reconnected */
        ...
        {                                        /* Attempt to connect */
            ...
            client.subscribe(HTCOMMANDTOPIC);
        }
        else
        {
...
void TaskPrintHT(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        //char ht[30];
        //snprintf(ht, 30, "H: %02f %%  T: %02f *C", h, t);
        ht = "H: " + String(h) + " %  " + "T: " + String(t) + " *C";
        Serial.println(ht);
        //publishMqtt(HTSENDTOPIC, ht); /* Didn't work, big error
    }
}
